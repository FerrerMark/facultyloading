...
// Handle faculty addition (from modal form submission)
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] === "add_faculty") {
    try {
        // Use $_POST directly to avoid filter_input issues
        $first_name = isset($_POST['firstname']) ? trim($_POST['firstname']) : '';
        $middle_name = isset($_POST['middlename']) ? trim($_POST['middlename']) : null; // Can be NULL
        $last_name = isset($_POST['lastname']) ? trim($_POST['lastname']) : '';
        $college = isset($_POST['college']) ? trim($_POST['college']) : null; // Can be NULL
        $employment_status = isset($_POST['employment_status']) ? trim($_POST['employment_status']) : null; // Can be NULL
        $address = isset($_POST['address']) ? trim($_POST['address']) : null; // Can be NULL
        $phone_no = isset($_POST['phone_no']) ? trim($_POST['phone_no']) : null; // Can be NULL
        $departmentID = isset($_POST['departmentID']) ? trim($_POST['departmentID']) : '';
        $role = isset($_POST['role']) ? trim($_POST['role']) : null; // Can be NULL
        $master_specialization = isset($_POST['master_specialization']) ? trim($_POST['master_specialization']) : null; // Can be NULL
        $max_weekly_hours = isset($_POST['max_weekly_hours']) ? (int)$_POST['max_weekly_hours'] : null; // Can be NULL
        $availability = isset($_POST['availability']) ? trim($_POST['availability']) : '';
        $start_time = isset($_POST['start_time']) ? trim($_POST['start_time']) : '';
        $end_time = isset($_POST['end_time']) ? trim($_POST['end_time']) : '';
        $request_id = isset($_POST['request_id']) ? (int)$_POST['request_id'] : 0;

        // Fix time format (append :00 to make it HH:MM:SS)
        if (!empty($start_time)) {
        ...
        // Execute the insert query and check if it succeeds
        $stmt->execute();

        // Update the stat column in faculty_requests to "Granted"
        ...
        // Log the error and redirect with the error message
...
<body>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <?php elseif ($request['status'] === "Approved" && ($request['stat'] ?? 'Pending') !== "Granted"): ?>
                                <button class="action-btn add-btn" onclick="openModal(<?php echo $request['request_id']; ?>)">+</button>
                            <?php else: ?>
    ...
    <script>
        function openModal(requestId) {
            ...
            // Set the selected request in the dropdown (for display)
        ...
        function closeModal() {
            ...
            // Reset the dropdown and hidden input
            requestSelect.value = '';
